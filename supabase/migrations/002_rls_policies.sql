...
CREATE POLICY "widgets_dashboard_owner" ON public.widgets
  FOR ALL
  USING (EXISTS (SELECT 1 FROM public.dashboards d WHERE d.id = dashboard_id AND d.user_id::text = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.dashboards d WHERE d.id = dashboard_id AND d.user_id::text = auth.uid()));
